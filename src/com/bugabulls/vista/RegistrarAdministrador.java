...
import com.bugabulls.controlador.InsertRegistrarAdministrador;
...
public class RegistrarAdministrador extends JDialog {
	...
	public RegistrarAdministrador(MenuAdministrador ma) {
                setModal(true);
		...
		JLabel jlFichaAdministrador = new JLabel("Ficha Administrador");
		jlFichaAdministrador.setOpaque(true);
		jlFichaAdministrador.setForeground(Color.WHITE);
		jlFichaAdministrador.setFont(new Font("Simplified Arabic", Font.BOLD, 18));
		jlFichaAdministrador.setBackground(new Color(128, 128, 0));
		jlFichaAdministrador.setBounds(0, 0, 644, 44);
		getContentPane().add(jlFichaAdministrador);
		...
		getContentPane().add(jlerror);
                ...
                asignarCodigoAdministrador();
                InsertRegistrarAdministrador ra = new InsertRegistrarAdministrador(this,codigoAdministrador);
                jbGuardar.addActionListener(ra);
	}
                public void asignarCodigoAdministrador(){
                    try {
                        ...
                        for (Administrador administrador : listaAdministradores) {
                            if(administrador.getCodigoadministrador()!=null){
                                codigoAdministrador=(administrador.getCodigoadministrador())+1;
                            }
                        }
                    } catch (Exception ex){
                        Logger.getLogger(RegistrarAdministrador.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    jtCodigo.setText(codigoAdministrador+"");
                }
}
